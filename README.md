...
This example shows how you can use the [Signature Drop Pre-Built Contract](https://docs.web3sdks.com/pre-built-contracts/signature-drop) to
utilize both the [claim](https://docs.web3sdks.com/pre-built-contracts/signature-drop#minting--claiming-nfts) function to claim NFTs under the criteria
of a claim condition, as well as the [Signature-based minting](https://docs.web3sdks.com/advanced-features/on-demand-minting) feature to
offer free NFT mints to specific users simultaneously.

![demo](demo.png)

We allow users who hold one of our [Early Access NFTs](https://opensea.io/collection/web3sdks-community) to mint the NFT for free, by generating a mint signature from the admin wallet on the server-side. Whereas users who _don't_ hold one of the NFTs do not qualify for the mint signature, but can still mint an NFT from the drop using the regular claim.

## Tools

- [Signature Drop](https://docs.web3sdks.com/pre-built-contracts/signature-drop) to lazy mint NFTs for other wallets to claim.
- [React SDK](https://docs.web3sdks.com/react) to connect to user's wallets and interact with the smart contract.

## Create Your Own

You can create a project using this example with the [web3sdks CLI](https://docs.web3sdks.com/web3sdks-cli) by running the below command:

```bash
npx web3sdks create --template signature-drop
```
...
We wrap our application in the `Web3sdksProvider` component in the [\_app.tsx](./pages/_app.tsx) file.
...
Now we can utilize all of the functionality of the [React SDK](https://docs.web3sdks.com/react) to connect to the user's wallets and their network on the [index.tsx](./pages/index.tsx) file.
...
const polygonSDK = new Web3sdksSDK("polygon");
...
```jsx
  ...
  const goerliSDK = Web3sdksSDK.fromPrivateKey(
